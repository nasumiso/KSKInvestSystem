...
def save_pf_kessan_db(stocks):
    ...
    #pf_kessan_dict = {k: {"kessanbi": v["kessanbi"], "stock_name": v["stock_name"], "code": v["code"], "kessan_quarter": get_kessan_quarter(v) } for k, v in stocks.items() if int(k) in code_list and v.get("kessanbi", "")}
    pf_kessan_dict = {k: {"kessanbi": v["kessanbi"], "stock_name": v["stock_name"], "code_s": get_db_code(v), "kessan_quarter": get_kessan_quarter(v) } \
    for k, v in stocks.items() if k in code_list_s+possess_list_s and v.get("kessanbi", "")}
    # 決算日でソート
...
def make_kessan_csv():
    ...
    # まず日付ごと
    day_kessan_dict = {}
    for k, v in pf_kessan_dict.items():
        kessanbi = v["kessanbi"]
        if not day_kessan_dict.has_key(kessanbi):
    ...
    current_list = []
    future_list = []
    for k, v in day_kessan_dict.items():
        if not k: # ''はスルー
            continue
    ...
    def write_to_csv(target_list):
        ...
        for date, codes in target_list:
            #expr = ",".join(["%d%s[%dQ]"%(l["code"], l["stock_name"], l.get("kessan_quarter", 0)) for l in codes])
            expr = ",".join(["%s%s[%dQ]"%(get_db_code(l), l["stock_name"], l.get("kessan_quarter", 0)) for l in codes])
    ...
    def write_to_csv_current(target_list):
        for date, codes in target_list:
            ...
            for l in codes:
                #csv_rec.append("%d%s[%dQ]"%(l["code"], l["stock_name"], l.get("kessan_quarter", 0)))
                csv_rec.append("%s%s[%dQ]"%(get_db_code(l), l["stock_name"], l.get("kessan_quarter", 0)))
