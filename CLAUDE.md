...
- スクリーンショット・検証画像は `.playwright-mcp/` 配下に保存する。Playwright MCP の `browser_take_screenshot` は `filename` に `.playwright-mcp/issue123-foo.png` のようにディレクトリ込みで指定する。`screencapture` 等の CLI も同じディレクトリへ出力。ルート直下に PNG を置かない (gitignore 済みだが散らかる)。
