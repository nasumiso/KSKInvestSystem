...
def todays_shintakane(upd=UPD_INTERVAL):
	...
	def create_already_list():
		...
		for i in range(BACK_DAY, 0, -1): # 昨日から
			...
			if os.path.exists(day_csv):
				...
				print "新高値銘柄%d個と出来高急増銘柄%d個を合成(%s)"%(len(day_list), len(day_list_d), day.date())
			...
			print "候補銘柄%d個"%len(day_list_filtered)
	...
	def create_today_list():
		...
		if today_data.endswith(".csv"):
			...
			print "新高値銘柄%d個と出来高急増銘柄%d個を合成"%(len(today_list), len(today_list_d))
	...
	print "決算更新追加:", len(kessan_lst_code), "個"
	print kessan_lst_code
	updatelist_all_code = already_list_code + today_list_code + kessan_lst_code # 更新を行う必要のあるすべての銘柄
	updatelist_all_code = list(set(updatelist_all_code))

	#---- マスターデータ更新
	# 最低購入代金取得のためdbを更新
	print "---> masterデータ更新(最低購入代金、銘柄概要取得のため)"
	stocks = stock_db.update_db_rows(updatelist_all_code, tables=["master"], upd=upd)
	print "<---- masterデータ更新完了"
	...
	def filter_today_list(lst):
		...
		def filter_dekidakaup(l):
			if not l.has_key("dekidaka_upratio"):
				return True
		...
		def filter_themes(l):
			...
			return themes != ""
		#print "候補数2:", len(lst)
		lst = [l for l in lst if filter_themes(l)]	
		#print "候補数3:", len(lst)
		return lst
	...
	today_already_list = [t for t in today_list if not t in today_only_list] # today_list-today_only_list
	already_only_list = [t for t in already_list if not t["code_s"] in today_list_code] # already_list-today_list

	print "本日銘柄%d [未調査 %d 調査済み%d] | 過去銘柄%d"%( len(today_list), len(today_only_list), len(today_already_list), len(already_only_list) )
	...
	def update_price_gyoseki_shihyo(today_list_all):
		...
		print ""
		print "----> 価格データ更新"
		stocks = stock_db.update_db_rows(today_list_all, tables=["price"], upd=upd)
		print "<---- 価格データ更新完了"
		...
		stocks = stock_db.update_db_rows(today_list_all, tables=["gyoseki"], upd=upd)
		...
		stocks = stock_db.update_db_rows(today_list_all, tables=["shihyo"], upd=upd)

		#---- 理論株価の更新
		print "="*20
		print "理論株価を更新します"
		print "="*20
		stocks = stock_db.update_db_rows(today_list_all, tables=["rironkabuka"], upd=upd)
	...
	def update_market():
		...
		shintakane_theme_csv = make_market_db.update_shintakane_theme_csv(stocks, today_code, past_code)
	...
	def puts_detail(d):
		...
		if TO_CSV:
			row = [code, stock_name, origin, market, sector, sector_detail, \
			kabuka, d["zenjitsuhi_per"], d["dekidaga"], purchase_money, market_cap, \
			vola, sell_press, signal, tags, kessanbi, \
			score_gyoseki, shihyo_pt, mom_pt, funda_pt, major_theme, overview]
			rows.append(row)
		else:
			try:
				print "%s %s [%s] %s %s(%s) %d(%s) %d | %d万 %d億"%\
				(code, name, origin, market, sector, sector_detail, 
	...
	if TO_CSV:
		...
		print "【本日銘柄】"
	# 今日のものを出力
	today_list_show = sorted(today_only_list, key=score_func, reverse=True) # 業績でソート
...
def convert_kabutan_dekidakaup_html(html):
	rows = []
	body = re.search(r'<table class="stock_table st_market">(.*?)</table>', html, re.S).group(0)
	#print body
	rank = 1
...
def convert_kabutan_shintakane_html(html):
	...
	for m in re.finditer(r'<td class="tac">(.*?)</td>.*?<th scope="row" class="tal">(.*?)</th>.*?<td class="tac">(.*?)</td>.*?<td>(.*?)</td>.*?<td>(.*?)</td>.*?<td class="w61">(.*?)</td.*?<td class="w50">(.*?)</td>', body, re.S):
		...
		try:
			zenjitsuhi = re.search(r'<span class="up">(.*)</span>', m.group(6)).group(1)
			zenjitsuhi_per = re.search(r'<span class="up">(.*)</span>', m.group(7)).group(1)+"%"
		except AttributeError:
			zenjitsuhi = 0
			zenjitsuhi_per = 0
...
def get_todays_dekidakaup():
	...
	try:
		...
		cach_dt = datetime(int(latest_date_m.group(1)), int(latest_date_m.group(2)), int(latest_date_m.group(3)))
	...
	for p in range(page_count):
		...
		html = http_get_html(URL_KABUTAN_DEKIDAKA+QUERY%(p+1), use_cache=useCache, cache_dir=cache_dir)
		htmls.append(html)
...
def get_todays_shintakane():
	...
	# 最初のページ
	htmls = []
	html = http_get_html(URL_KABUTAN_SHINTAKANE+QUERY%1, use_cache=useCache, cache_dir=cache_dir)
	htmls.append(html)
	# 更新日付
	date_m = re.search(r'<div class="meigara_count">.*(\d\d\d\d)年(\d\d)月(\d\d)日.*?</div>', html, re.S) # re.S:改行を含む
...
def parse_kessan_html(html):
	...
	print "決算修正:", [l[:2] for l in mod_lst]
...
def update_todays_kessan():
	...
	for p in range(page_max):
		...
		if os.path.exists(cache_path):
			...
			if os.path.exists(cache_path) and tdy_date <= cache_date:
				use_cache = True
			print "%dページ決算発表キャッシュ日付:%s キャッシュ:%s"%(page, cache_date, use_cache)

		kessan_html = http_get_html(TODAYS_KESSAN_URL, use_cache=use_cache, \
		...
		print "ページ:%d 今読んでいる決算日付:%s ここまで取得日:%s"%(page, current_day, kessan_ge_day)		
	...
	for item in announce_lst:
		...
		try:
			...
			print code_s, "はDBにありません"
			continue
		print "決算発表:", code_s, stocks[code_s].get("stock_name","銘柄名不明"), announce_date, item[3]
