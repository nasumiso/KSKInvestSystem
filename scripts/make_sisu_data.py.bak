...
ASSET_CLASSES = [JP_STOCK, JP_REIT, GL_STOCK, GL_REIT, EM_STOCK, GL_BOND, GOLD, MY_FRONTIER]
...
def parse_html_yahoo_us(html, is_all_price=False):
	...
	for m2 in re.finditer(r'<tr><td class="yfnc_tabledata1".+?</tr>', html):
		...
		if m:
			...
			if is_all_price:
				prices = [float(m.group(5).replace(",", "")), 
...
def modify_distribute(code_name, rows):
	if DISTRIBUTE_DATA.has_key(code_name):
...
def make_rs_db():
	...
	for i, url in enumerate(ASSET_URL2):
		...
		# 一年前の日付でリクエスト
		today = date.today()
		url = url%("%04d%02d%02d"%(today.year-1,today.month,today.day), \
