...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "抽出できていないと考えてください。"
        "決算資料の読み方は用途で選んでください。"
        "(1) PDF がすでにチャットに添付されている、またはこのサーバーと同じ端末で"
        "ファイルを読める (local_path を開ける) なら、それを使い、取りに行き直さないで"
        "ください。"
...
def list_earnings_documents_data(
    ...
    # 中計は会社IRページから手動で集めたものだけで、上の網羅性 (TDnet の収集期間) とは
    # 無関係。0件を「中計は無い」と読ませないよう、常に網羅性は未確認として返す。
    if doc_type == "chuki_plan":
        partial = True
        notes = [
            "中期経営計画は会社IRページから手動で取得したものだけを返します。"
            "0件でも、この銘柄に中計が存在しないことを意味しません。"
        ]
