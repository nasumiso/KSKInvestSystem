...
この濃淡を利用側へ数値で渡す仕組みは issue #461 で検討中。

### PDF は MCP では渡せない

4.9MB の PDF は base64 で約6.5MB となりコンテキストに載らず、ChatGPT のコネクタも
ツール結果をテキストとして扱う。返すのは `local_path` (サーバーを動かしている端末の
パス) のみで、**LLM がそれを開くことはできない**。

PDF が必要なときは **LLM がユーザーに添付を依頼し、ユーザーが手で添付する**。
添付は正規のアップロード経路を通るためビジョン処理が効き、画像主体の PDF も読める
(2026-08-30 実機確認済み)。つまり PDF を見る経路には必ず人が1手挟まる。
