...
def get_from_kabutan3(html):
    ...
    # ------------------------------
    year_tbl_m = re.search(
        r'<div class="cap1"><h3>財務 【実績】</h3></div>.*?<table>(.*?)</table>',
        html,
        re.S,
    )
    if not year_tbl_m:
...
def calc_theory_price(bps, eps, equity_ratio, price=0, preceding_eps=None):
    ...
    discount_rate = step_func(
        equity_ratio, [0, 10, 33, 50, 67, 80], [50, 60, 65, 70, 75, 80]
    )
    ...
    if mbps > 0:
        ...
        roa = 0
    roa = min(roa, 0.3)
    zaimu_correction = step_func(equity_ratio, [33, 40, 50, 66], [1.5, 1.36, 1.2, 1])
    ...
    if preceding_enterprise_value is not None:
        theory_price_preceding = (
            asset_value + preceding_enterprise_value
        ) * risk_correction
    ...
    return (
        max(int(theory_price), 0),
        max(int(theory_price_up), 0),
        max(int(theory_price_down), 0),
        theory_price_preceding,
    )
...
def analyze_from_kabutan(code_s, upd=UPD_INTERVAL, stock=None):
    ...
    if dic:
        ...
        if stock:
            ...
            preceding_eps = get_preceding_eps(
                dic["mod_eps"],
                progress.get("quarter"),
                progress.get("profit"),
                progress.get("profit_pre"),
            )
        # 必要な値を渡して理論株価計算
        try:
            theory_price = calc_theory_price(
                dic["bps"],
                dic["mod_eps"],
                dic["equity_ratio"],
                dic["price"],
...
def get_rironkabuka_kairi_fromprice(
    theory_price, theory_price_up, theory_price_down, therory_price_preceding, price
):
    """理論株価乖離率の計算
    Args:
...
def get_rironkabuka_kairi(stock):
    ...
    kairi_lst = get_rironkabuka_kairi_fromprice(
        theory_price, theory_price_up, theory_price_down, theory_price_preceding, price
    )
    return kairi_lst
...
def calc_theory_pt(code_s, stock=None):
    ...
    log_debug(
        ...
        % (
            theroy,
            str(theory_proceding) if theory_proceding is not None else "-",
            theory_up,
            theory_down,
        )
    )
    ...
    if not (theory_price[4] == 0) and theory_price[0] != 0:  # 価格がある
        ...
        # 実際のポイント計算
        theory_pt = (
            step_func(theroy, [0, 20, 40, 60, 100], [20, 40, 60, 80, 100], 0)
            * THEORY_MAX
            / 100
        )
        # theory_proceding_pt = step_func(theory_proceding, [0, 20, 40, 60, 100], [20, 40, 60, 80, 100], 0)*THEORY_PROCEDING_MAX/100
        theory_up_pt = (
            step_func(theory_up, [30, 60, 100, 150, 300], [20, 40, 60, 80, 100], 0)
            * THEORY_UP_MAX
            / 100
        )
        theory_down_pt = (
            step_func(theory_down, [-60, -30, 0, 30], [25, 50, 75, 100], 0)
            * THEORY_DOWN_MAX
            / 100
        )
    else:
...
def get_rironkabuka_expr2(record, price):
    # 理論株価文字列表現テスト用
    kairi_list = get_rironkabuka_kairi_fromprice(
        record["rironkabuka"],
        record["rironkabuka_up"],
        record["rironkabuka_down"],
        record["rironkabuka_preceding"],
        price,
    )
