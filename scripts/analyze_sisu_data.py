...
def buy_and_hold():
    ...
    if not rebaranlce:
        ...
        print(gain_jp_stock, gain_jp_reit, gain_gl_stock, gain_gl_reit, \
        gain_em_stock, gain_gl_bond, gain_gold)
        asset_rate = [20, 5, 20, 10, 10, 25, 10]
        gain_list = [gain_jp_stock, gain_jp_reit, gain_gl_stock, gain_gl_reit, \
        gain_em_stock, gain_gl_bond, gain_gold]
...
def rs_3612ma():
    ...
    for i, _row in enumerate(db_dict[JP_STOCK]):
        ...
        sell = []
        have = []
        for j, (cur, prev, asset) in enumerate(zip(asset_rate, asser_rate_prev, ASSET_CLASSES)):
            if cur > prev:
        ...
        if i%52==0: # 年単位
            ...
            try:
                ...
                print("年利益：%.2f(%.1f<-%.1f) %d%% input:%d"%(profit, total_return_current, \
                    total_return_prev, \
                    100*total_return_current/total_return_prev-100, input_money_current))
            except ZeroDivisionError as e:
                print("年利益：0")
        print("return1:", [round(a, 3) for a in asset_return_1])
        print("asset_total_before:", [round(a, 1) for a in asset_total])
        asset_total = [t*r1 for t, r1 in zip(asset_total, asset_return_1)] # 1ヶ月リターン反映
        total_return = sum(asset_total)
        if tax and i%52==0:
            if profit > 0:
    ...
    print("RS_MA3-6-12：%d (%d/%d)"%(int(total_return/input_money*100), \
        total_return, input_money))
...
def rs_macd():
    ...
    for asset in ASSET_CLASSES:
        ...
        for i, row in enumerate(price_db):
            ...
            if i<n:
                ...
                ema12[i] = ema12[i-1]+(2.0/(n+1))*(price_only[i]-ema12[i-1])
            ...
            if i<n:
                ...
                ema26[i] = ema26[i-1]+(2.0/(n+1))*(price_only[i]-ema26[i-1])
            # first = average(price_only[max(i-2*n+1,0):max(i-n+1,1)])
            # for j in range(max(i-n+1,1),i+1):
            # 	first += (2.0/(n+1))*(price_only[j]-first)
            # ema26[i] = first
    ...
    for i, _row in enumerate(db_dict[JP_STOCK]):
        ...
        for j, asset in enumerate(ASSET_CLASSES):
            row = db_dict[asset][i]
            month_signals = [row[2], db_dict[asset][max(i-1,0)][2], \
            db_dict[asset][max(i-2,0)][2], db_dict[asset][max(i-3,0)][2]]
            if row[2] == 2:
...
def latest_3612ma():
    ...
    for i, asset in enumerate(ASSET_CLASSES):
        print("%10s %+5.1f %+5.1f %+5.1f %+5.1f %+5.1f %5s %2d"%(asset, return1[i], return3[i], return6[i],\
