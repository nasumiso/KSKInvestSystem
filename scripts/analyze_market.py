...
def analyze_market():
	...
	print("%d(%+.1f%%) %d "%(n225_current, (n225_data[0][4]-n225_data[1][4])*100/n225_data[1][4], n225_ma10), end=' ')
	print("SP_RATIO(20,10)=(%.1f %.1f) R_VOL=%.2f"%tuple(n225_sp_ratio))
	print("%sダウ　　:(10ma %+.1f%%) "%(dow_mark, (float)(dow_current-dow_ma10)*100/dow_ma10), end=' ') 
	print("%d(%+.1f%%) %d "%(dow_current, (dow_data[0][4]-dow_data[1][4])*100/dow_data[1][4], dow_ma10), end=' ')
...
def analyze_shintakane():
	...
	# unknownを除く
	sector_newhigh_list = {k:v for k,v in list(sector_newhigh_list.items()) \
	...
	print("="*30)
	# 新高値セクター解析結果を表示
	for key, val in reversed(sorted( list(sector_newhigh_list.items()), \
		key=comp_val )):
		print(key, val[0][0], val[1][0], val[2][0], "/%d"%\
		len(sector_tables[key]) if key in sector_tables else 0)
