...
def is_latest_info(url):
    ...
    if os.path.exists(cach_path):
        ...
        if (
            date(now_dt.year, now_dt.month, now_dt.day)
            - date(cache_dt.year, cache_dt.month, cache_dt.day)
        ).days > 0:
...
def get_n225_data():
    ...
    html = http_get_html(
        URL_N225, cache_dir="market_data", use_cache=use_cache
    )  # latest=True
    ...
    # 本日の値
    m = re.search(
        r'<span class="time_rtq_ticker"><span id=".*?">(.*?)</span></span>', html
    )
...
def get_dow_data():
    ...
    html = http_get_html(
        URL_DOW, cache_dir="market_data", use_cache=use_cache
    )  # latest=True
    is_all_price = True
    price_list = make_sisu_data.parse_html_yahoo_us(html, is_all_price=is_all_price)
    m = re.search(
...
def analyze_market():
    ...
    print(
        ...
        % (
            n225_current,
            (n225_data[0][4] - n225_data[1][4]) * 100 / n225_data[1][4],
            n225_ma10,
        ),
        end=" ",
    )
    ...
    print(
        ...
        % (
            dow_current,
            (dow_data[0][4] - dow_data[1][4]) * 100 / dow_data[1][4],
            dow_ma10,
        ),
        end=" ",
    )
...
def analyze_shintakane():
    ...
    sector_newhigh_list = {
        k: v
        for k, v in list(sector_newhigh_list.items())
    ...
    for key, val in reversed(sorted(list(sector_newhigh_list.items()), key=comp_val)):
        print(
            ...
            val[1][0],
            val[2][0],
            "/%d" % len(sector_tables[key]) if key in sector_tables else 0,
        )
        code_name = [
