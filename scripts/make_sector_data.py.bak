...
def get_sector_detail(code_s):
	...
	for sector_name, code_list in sector_tables.items():
...
def test_make_secotr_data():
	...
	# 表示
	for sector_name, code_list in sector_table.items():
		unknown_code_list = []
		print "-"*5, sector_name, "%d銘柄"%len(code_list)
		for code_s in code_list:
			if stocks_db.has_key(code_s):
...
def update_sector_stockdb():
	...
	for code_s in stocks.iterkeys():
		stock = stocks[code_s]
		if stock.has_key("sector_detail"):
			print "Unknown:", code_s, stock["sector_detail"]
			continue # unkownとして更新する場合はコメントアウト
		# 詳細セクター名更新
		for sector_name, code_s_list in sector_tables.items():
			if code_s in code_s_list:
				stocks[code_s]["sector_detail"] = sector_name
				print "詳細セクター更新: %s%s <- %s"%(code_s, stock["stock_name"], sector_name)
				break
