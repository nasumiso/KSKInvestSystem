...
def has_stock_data(stocks, code_s, latest=False):
	...
	INTERVAL_DAY = 7 #10
	#code = int(code)
	if stocks.has_key(code_s):
		if stocks[code_s].has_key("stock_name"):
			if latest: # 最新だ
...
def get_stock_data(stocks, code_s, upd=UPD_INTERVAL):
	...
	# DBにある場合はそれを返す
	if stocks.has_key(code_s) and upd < UPD_INTERVAL:
		if stocks[code_s].has_key("stock_name"):
...
def has_price_data(stocks, code_s, latest=False):
	...
	#code = int(code)
	if stocks.has_key(code_s):
		if stocks[code_s].has_key("sell_pressure_ratio"):
			if latest:
...
def get_price_data(stocks, code_s, upd=UPD_INTERVAL):
	...
	if stocks.has_key(code_s): #and not latest: #デバッグ用強制
		if stocks[code_s].has_key("access_date_price") and upd < UPD_INTERVAL:
	...
	themes = stocks[code_s].get("themes","") if stocks.has_key(code_s) else None
	if themes:
...
def has_active_dbdata(stocks, code_s, access_key, interval_day, latest):
	...
	if stocks.has_key(code_s):
		...
		if stocks[code_s].has_key(access_key):
...
def get_gyoseki_data(stocks, code_s, upd=UPD_INTERVAL):
	...
	#code = int(code)
	if stocks.has_key(code_s):
		if stocks[code_s].has_key("access_date_gyoseki") and upd < UPD_INTERVAL:
...
def get_rironkabuka_data(stocks, code_s, upd=UPD_INTERVAL):
	...
	#code = int(code)
	if stocks.has_key(code_s):
		if stocks[code_s].has_key("access_date_rironkabuka") and upd < UPD_INTERVAL:
			print "DBから理論株価情報を取得します"
			return stocks[code_s]
	stock = stocks[code_s] if stocks.has_key(code_s) else None
...
def has_shihyo_data(stocks, code_s, latest=False):
	...
	if stocks.has_key(code_s):
		...
		if stocks[code_s].has_key("access_date_shihyo"):
			if latest:
...
def get_shihyo_data(stocks, code_s, upd=UPD_INTERVAL):
	...
	if stocks.has_key(code_s):
		if stocks[code_s].has_key("access_date_shihyo") and not latest:
...
def update_db(stocks, stock_data):
	...
	#print stock_data
	# 更新
	if not stock_data.has_key("code_s"):
		if not stock_data.has_key("code"):
	...
	print "DB更新しました: ", code_s, stock_data.keys()
	# 更新後のカラム表示
...
def get_trend_template_expr(stock):
	...
	"""
	if not stock.has_key("trend_template"):
		return "-"
...
def make_signal(stock):
	...
	if new_high:
		if stock.has_key("access_date_price"):
...
def get_access_dates_expr(stock_data):
	...
	"""
	date = ""
	if stock_data.has_key("access_date_gyoseki"):
		dt = stock_data["access_date_gyoseki"]
		date = "%s/%s"%(dt.month,dt.day)
		month = dt.month
	date_sh = ""
	if stock_data.has_key("access_date_shihyo"):
		...
		if month == dt.month:
			...
			month = dt.month
	date_pr = ""
	if stock_data.has_key("access_date_price"):
...
def list_all_db(upload_csv=True, update_portforio=True):
	...
	for i, stock in enumerate(stocks_active):
		...
		overview = ""
		if stock_data.has_key("overview"):
...
def delete_db_column(stocks, column):
	for k, stock in stocks.iteritems():
		if stock.has_key(column):
			del stock[column]
		#print_dict(stock)	
...
def convert_code_db():
	...
	stocks = {}
	for code, stock in stocks_bk.items():
