...
- **シグナル強度（webapp 表示, issue #253）**: 一覧/詳細の signal セルは tooltip に検出日・強度・意味を出し、強度×鮮度で赤系背景を濃淡表示する。詳細チャートには発生日のポ=緑三角・ブ=橙ダイヤのマーカーを X軸直上の固定バンド（ポ下段・ブ上段、株価/RS線と分離して最前面）に表示する。X は週足だが発生日（日単位）を週バー間で日割り按分、サイズは強度連動（ポ・ブ共通）。強度しきい値（暫定）— ポ: MA10乖離 `≧-1`強/`-3〜-1`中/`<-3`弱、ブ: 出来高超過率 `≧200`強/`100〜200`中/`<100`弱。鮮度: 0-2日濃/3-5日中/6-7日薄。
