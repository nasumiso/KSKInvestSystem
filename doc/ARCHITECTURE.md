...
`shintakane.py` → `make_stock_db.py` → `run_theme_news.py` を逐次実行。theme-news は `make_stock_db.py` の結果サマリーを出した後、`make_stock_db.py` 成功時のみ実行する。開発時は `bash shintakane_cron.sh --skip-theme-news` で theme-news を省略できる。macOS launchd（`deploy/com.k_sohara.shintakane.cron.plist`）で定期実行。運用機での構築手順は [OPERATIONS.md](OPERATIONS.md) を参照。
