...
# (実データ約17MBに対し過去7.35GBまで膨張)。
# #451 で全工程が排他ロックされたため、WebApp や日次バッチを止めずに実行してよい。
# 運用機では金曜の日次バッチ末尾で自動実行される (doc/OPERATIONS.md)。
...
`shintakane_cron.sh` が `shintakane.py` → `make_stock_db.py` → `exposure_guide.py` → `run_theme_news.py` を逐次実行。macOS launchd（`deploy/com.k_sohara.shintakane.cron.plist`）で平日19:00に定期実行。運用機での構築手順は [OPERATIONS.md](OPERATIONS.md) を参照。
